 ...
 * code n'a pas terminé de s'éxecuter.
 */
...
// finally ne récupère rien, j'aurais donc du utiliser ()=> plutôt que variables=>
request.finally(test=>console.log("finally",test));

/**
 * Il est possible de résoudre plusieurs promesses en même temps.
 * Pour cela on fera appel à la methode .all() de l'objet Promise à laquelle on
...
Promise.all([r1, r2]).then(res=>{
    ...
    res.forEach(r=>{
        // Et je traite chaque réponse comme je le ferais sur fetch classique.
        if(r.ok){
 ...
 * elles aussi un tableau de prommesse.
 ...
 * .any() lancera la catch, si toute les promesses retourne une erreur.
 */
/**
