...
// JS accepte d'appeler une fonction qui attend des arguments, même sans lui en donner les arguments seront alors undefined
bonsoir();
...
function BonneNuit(nom1, nom2){
    ...
    console.log("%cBonne nuit "+nom1+" et " + nom2, "color: green; font-size:5rem;");
}
/* Si une fonction attend plusieurs arguments, alors lors de l'appel, il faudra lui sonner plusieurs arguments séparés d'une virgule. 
La première valeur donné ira dans le premier argument, la seconde valeur dans le second et ainsi de suite. */
BonneNuit("Patrick", "Raphael");

function goodBye(nom1, nom2="les autres"){
    /* Il est possible de donner des valeurs par défaut à nos paramètres dans ce cas, si on lui donne une valeur, alors il prendra cette valeur
    mais si on lui donne rien, alors il prendra sa valeur par défaut. */
    console.log("Good bye "+nom1 +" et "+ nom2);
}
// Affiche Kevin et les autres.
goodBye("Kevin");
// Affiche Kevin et Dimitri.
goodBye("Kevin", "Dimitri");

function goodMorning(...noms){
    /* Le rest operator (...argument) permet de ranger tous les arguments donné dans un tableau. */
...
goodMorning("Antoine", "Daniel", "Charle", "Louis");
...
/* cernatines fonctions, comme forEach, prennent en argument une autre fonction. Par exemple forEach appelle la fonction donné en argument, pour chaque élément du tableau. */
pr.forEach(bonsoir);
/* On appelle cela une fonction callback
Cela peut être une fonction déclaré précédement, comme une fonction directement écrite entre les parenthèses de forEach, on parle alors de fonction anonyme */
